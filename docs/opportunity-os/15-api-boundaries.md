...
| **Founder Intent / Missions / Engineering Director** *(Phase 4.1–4.2)* | `engine/founder-intent` | `prepareChiefOfStaffPlan()`, `analyzeFounderIntent()`, `interpretFounderIntent()`, `completeMissionFromClarification()`, `validateMission()`, `saveMission()`, `getActiveMissions()`, `generateMissionStrategy()`, `recommendEngineeringTasks()`, `parseMasterBacklog()`, `selectNextBacklogTask()`, `createBuilderPlanFromBacklogTask()`, `createPressureWashingMissionTemplate()`, `createKtmMissionTemplate()`, `createApartmentWorkshopMissionTemplate()`, `listBusinessOperatorMissionTemplates()`, `alignOpportunityToMission()`, `buildFounderBriefing()` | CEO intent → validated plans + `runtime/missions/` + read-only reports + backlog-derived Builder Plans + business operator mission templates — **must not** call pipeline execution, OpenClaw execution, or outreach |
