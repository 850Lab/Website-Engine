...
| **Founder Intent / Missions** *(Phase 4.1)* | `engine/founder-intent` | `prepareChiefOfStaffPlan()`, `analyzeFounderIntent()`, `interpretFounderIntent()`, `completeMissionFromClarification()`, `validateMission()`, `saveMission()`, `getActiveMissions()`, `generateMissionStrategy()`, `recommendEngineeringTasks()`, `alignOpportunityToMission()`, `buildFounderBriefing()` | CEO intent → validated plans + `runtime/missions/` + read-only reports — **must not** call pipeline execution |
