...
| **Founder Intent / Missions / Engineering Director** *(Phase 4.1–4.2)* | `engine/founder-intent` | `prepareChiefOfStaffPlan()`, `analyzeFounderIntent()`, `interpretFounderIntent()`, `completeMissionFromClarification()`, `validateMission()`, `saveMission()`, `getActiveMissions()`, `generateMissionStrategy()`, `recommendEngineeringTasks()`, `createEngineeringValidationPlan()`, `parseMasterBacklog()`, `selectNextBacklogTask()`, `createBuilderPlanFromBacklogTask()`, `saveEngineeringTask()`, `listEngineeringTasks()`, `approveEngineeringTask()`, `activateEngineeringTask()`, `blockEngineeringTask()`, `completeEngineeringTask()`, `createOpenClawHandoffPackage()`, `validateOpenClawHandoffPackage()`, `createPressureWashingMissionTemplate()`, `createKtmMissionTemplate()`, `createApartmentWorkshopMissionTemplate()`, `createWebsiteAgencyMissionTemplate()`, `listBusinessOperatorMissionTemplates()`, `alignOpportunityToMission()`, `buildFounderBriefing()` | CEO intent → validated plans + `runtime/missions/` + read-only reports + backlog-derived Builder Plans + regression-aware validation plans + runtime engineering-task lifecycle + handoff-only OpenClaw job drafts + business operator mission templates — **must not** call pipeline execution, OpenClaw workers, job dispatch, or outreach |
...
| **Live Sensors** *(Phase 3.5 + C1 + O-WEB2)* | `engine/sensors/live` | `collectFileDropObservations()`, `runFileDropSensor()`, `collectLegacyWebsiteScanObservations()`, `runWebsiteScanBridgeSensor()` | File drop / legacy website scan → Observation → Signal Registry only; mission hints/source labels and legacy website scan provenance may be carried as metadata/provenance but must not create downstream artifacts, contacts, campaigns, or outreach |
...
| **Chaining** | Orchestrator (3.6) enqueues downstream jobs from events — not inline calls |
...
|---|---|---|
| `signal.created` | Signal registry / ingest | `fact.build` |
| `facts.completed` | Fact builder handler (3.7) | `graph.project` |
| `graph.completed` | Graph handler (3.7) | `situation.build` |
| `situations.completed` | Situation handler (3.7) | `hypothesis.generate` |
| `hypotheses.completed` | Hypothesis handler (3.7) | `problem.infer` |
| `problems.completed` | Problem handler (3.7) | `capability.match` |
| `capability.completed` | Capability handler (3.7) | `offer.recommend` |
| `offer.completed` | Offer handler (3.7) | `opportunity.build` |
...
| **OpenClaw** *(Phase 3.1.8)* | [29-openclaw-constitution.md](./29-openclaw-constitution.md) · `engine/openclaw` · `scripts/openclaw/` | Builder implements approved scope; QA validates read-only; both one Job per CLI | Redesign architecture; QA commits; bypass owner gates; MC/Score Council changes |
