...
| **Founder Intent / Missions / Engineering Director** *(Phase 4.1–4.2)* | `engine/founder-intent` | `prepareChiefOfStaffPlan()`, `analyzeFounderIntent()`, `interpretFounderIntent()`, `completeMissionFromClarification()`, `validateMission()`, `saveMission()`, `getActiveMissions()`, `generateMissionStrategy()`, `recommendEngineeringTasks()`, `parseMasterBacklog()`, `selectNextBacklogTask()`, `createBuilderPlanFromBacklogTask()`, `saveEngineeringTask()`, `listEngineeringTasks()`, `approveEngineeringTask()`, `activateEngineeringTask()`, `blockEngineeringTask()`, `completeEngineeringTask()`, `createPressureWashingMissionTemplate()`, `createKtmMissionTemplate()`, `createApartmentWorkshopMissionTemplate()`, `createWebsiteAgencyMissionTemplate()`, `listBusinessOperatorMissionTemplates()`, `alignOpportunityToMission()`, `buildFounderBriefing()` | CEO intent → validated plans + `runtime/missions/` + read-only reports + backlog-derived Builder Plans + runtime engineering-task lifecycle + business operator mission templates — **must not** call pipeline execution, OpenClaw execution, or outreach |
