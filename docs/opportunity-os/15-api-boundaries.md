...
| **OpenClaw** *(Phase 3.1.7.5)* | `engine/openclaw` + `engine-data/openclaw/prompts/` + `scripts/openclaw/` | `hashCanonicalPromptText()`, `verifyOwnerApproval()`, `verifyOpenClawIdempotency()`, `validateCommandAllowlist()`, `runOpenClawBuilderJob()` · CLI: `run-builder-job.js` | Prompt hash verified; idempotency bound; command allowlist; VALIDATION_DEMO gated |
...
| **OpenClaw** *(Phase 3.1.7.5)* | [29-openclaw-constitution.md](./29-openclaw-constitution.md) · `engine/openclaw` · `engine-data/openclaw/prompts/` · `scripts/openclaw/` | Claim one approved Job with verified promptHash; allowlisted commands only; emit full event trail | Redesign architecture; outreach; bypass owner gates; MC/Score Council changes |
