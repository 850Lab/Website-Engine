...
| `B4` | `complete` | `createEngineeringValidationPlan()` implemented in `src/engine/founder-intent/engineering-director.js`; Engineering Director task recommendations and backlog-derived Builder Plans include focused validators, phase validators, regression validators, core validation, affected modules, and failure/repair policy before implementation. | `node scripts/opportunity-engine/validate-engineering-director.js` |
| `B5` | `complete` | `backlog-progress-dashboard.js` generates deterministic JSON/Markdown progress from this backlog, including overall/category completion, current task selection, blocked task IDs, and remaining task/commit/hour estimates; generated reports are gitignored and reporting-only. | `node scripts/opportunity-engine/validate-backlog-progress-dashboard.js` |
...
| `O-WEB2` | `complete` | `collectLegacyWebsiteScanObservations()` and `runWebsiteScanBridgeSensor()` implemented in `src/engine/sensors/live/website-scan-bridge.js`; tracked legacy website scan records become mission-aware OS observations/signals with provenance and no downstream facts, situations, problems, opportunities, contacts, campaigns, or outreach. | `node scripts/opportunity-engine/validate-website-operator.js` |
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| `A1` | Founder conversation state | Persist multi-turn Founder conversations, clarification state, and active intent context without executing business actions. | Founder can express goals naturally over time. | P0 | Phase 4.1 mission registry | `validate-phase-4-2.js` | M | L | Privacy policy published at `/privacy`; asks to send outreach | `src/engine/founder-intent/`, `runtime/chief-of-staff/` | Conversations resume, clarification state is auditable, no pipeline execution. |
...
| `B5` | Backlog progress dashboard | Generate living progress reports from the Master Engineering Backlog and execution status. | Gives the Founder and Engineering Director current progress, current task, blockers, and remaining effort without hand-maintained status math. | P1 | B1, B4 | `validate-backlog-progress-dashboard.js` | S | L | Dashboard tries to become UI, daemon, or execution loop | `scripts/opportunity-engine/`, `reports/`, `docs/opportunity-os/` | JSON/Markdown dashboard is deterministic, gitignored, backlog-derived, and reporting-only. |
